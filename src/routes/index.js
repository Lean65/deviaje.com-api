...
router.get('/findcode', (req, res)=>{
    ...
    axios.get(`${BASE}/locations/query?term=${term}`, {headers: {
        apikey: KEY
    }})
...
async function FindCode(name){
    const KEY = 'lOcmY9Q0RrcW078bg5nzA-nMzQUEbrHB'
    try {
        const resp = await axios.get(`${BASE}/locations/query?term=${name}`, {headers: {
            apikey: KEY
        }})
...
router.get('/topdistinations', (req, res)=>{
    ...
    axios.get(`${BASE}/locations/topdestinations?term=${term || 'london_gb'}`, {headers: {
        apikey: 'n_-RwJB-98J-s0_OyVx1n9tFSd5SPtoI'
    }})
